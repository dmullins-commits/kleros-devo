...
export default function NewTeamReport({ filterType, teamId, team, classPeriod, metrics, records, athletes, organization, onBack, initialGraphs, dateRangeType, startDate }) {
  ...
  const summaryData = allSelectedMetrics.map(metric => {
    const metricRecords = records.filter(r =>
      athleteIds.includes(r.athlete_id) && r.metric_id === metric.id
    );

    if (metricRecords.length === 0) return null;

    const latestRecords = metricRecords
      .sort((a, b) => new Date(b.recorded_date) - new Date(a.recorded_date))
      .slice(0, filteredAthletes.length);

    const avgLatest = latestRecords.reduce((sum, r) => sum + r.value, 0) / latestRecords.length;
    const allPRs = filteredAthletes.map(athlete => {
    ...
    const avgPR = allPRs.length > 0 ? allPRs.reduce((sum, v) => sum + v, 0) / allPRs.length : null;

    return { metric, avgLatest, avgPR };
  }).filter(Boolean);
  ...
  const renderGraph = (graph, index, genderLabel, genderAthletes) => {
    ...
    const teamRecords = records.filter(r =>
      genderIds.includes(r.athlete_id) && graph.metricIds.includes(r.metric_id)
    );

    // Use full date range for last30 or custom, otherwise just dates with data
    let sortedDates;
    ...
    const chartData = sortedDates.map(date => {
      ...
      graphMetrics.forEach(metric => {
        const dayRecords = teamRecords.filter(r => r.recorded_date === date && r.metric_id === metric.id);
    ...
    return (
      <div key={`${graph.id}-${genderLabel}`} className="avoid-break">
        <Card className="bg-gray-950 border border-gray-800">
          <CardHeader className="border-b border-gray-800">
            <CardTitle className="text-white">
              {graph.title || `Graph ${index + 1}`} - {genderLabel}
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            {chartData.some(d => Object.keys(d).length > 1) ? (
              ...
              <div className="text-center py-12">
                <p className="text-gray-400">No data available for {genderLabel}</p>
              </div>
            )}
          </CardContent>
        </Card>
    ...
    <>
      ...
      <div id="report-content" className="space-y-6">
        ...
        <Card className="bg-gray-950 border border-gray-800 avoid-break">
          <CardHeader className="border-b border-gray-800">
            <CardTitle className="text-white text-2xl">
              {filterType === "team" ? "Team" : "Class"} Performance Report
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6">
        ...
        {summaryData.length > 0 && (
          <Card className="bg-gray-950 border border-gray-800 avoid-break">
            <CardHeader className="border-b border-gray-800">
              <CardTitle className="text-white">Selected Metrics Summary (Team Average)</CardTitle>
            </CardHeader>
            <CardContent className="p-6">
                ...
                <table className="w-full">
                  <thead className="bg-gray-800">
                    <tr>
                      ...
                      <th className="text-left p-4 text-gray-300 font-bold">Avg PR</th>
                      <th className="text-left p-4 text-gray-300 font-bold">Avg Most Recent</th>
                    </tr>
                  </thead>
                  <tbody>
                    {summaryData.map((row, index) => (
                      <tr key={row.metric.id} className={index % 2 === 0 ? 'bg-gray-900' : 'bg-gray-950'}>
                        ...
                        <td className="p-4 text-gray-300">
                          {row.avgPR ? row.avgPR.toFixed(row.metric.decimal_places ?? 2) : 'N/A'}
                        </td>
                        <td className="p-4 text-gray-300">
                          {row.avgLatest.toFixed(row.metric.decimal_places ?? 2)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className="space-y-6">
          {graphs.map((graph, index) => (
            <div key={graph.id} className="space-y-6">
              <div className="flex justify-between items-center print-hide">
                <input
                  type="text"
                  value={graph.title || `Graph ${index + 1}`}
                  onChange={(e) => updateGraphTitle(graph.id, e.target.value)}
                  className="text-xl font-bold text-white bg-transparent border-none outline-none focus:ring-2 focus:ring-blue-400 rounded px-2 py-1"
              ...
              <div className="hidden print:block">
                <h3 className="text-xl font-bold text-white mb-4">{graph.title || `Graph ${index + 1}`}</h3>
              </div>
              {maleAthletes.length > 0 && renderGraph(graph, index, "Male Athletes", maleAthletes)}
              {femaleAthletes.length > 0 && renderGraph(graph, index, "Female Athletes", femaleAthletes)}
            </div>
          ))}
        </div>
